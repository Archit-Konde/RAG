...
def make_dense_results(n: int) -> list:
    ...
    return [
        {"text": f"dense_doc_{i}", "metadata": {"chunk_index": i, "source": "test.txt"},
...
def make_sparse_results(n: int, offset: int = 0) -> list:
    ...
    return [
        {"text": f"sparse_doc_{i+offset}", "score": 1.0 - i * 0.1, "index": i + offset}
        for i in range(n)
    ]
...
def test_rrf_score_formula():
    ...
    top = fused[0]
    assert top["index"] == 0
    assert abs(top["rrf_score"] - expected_0) < 1e-10, (
        f"Expected {expected_0:.8f}, got {top['rrf_score']:.8f}"
    )
...
def test_deduplication():
    ...
    dense = [{"text": "doc_0", "metadata": {}, "score": 0.9, "index": 0},
             {"text": "doc_1", "metadata": {}, "score": 0.8, "index": 1}]
    sparse = [{"text": "doc_0", "score": 0.7, "index": 0},
              {"text": "doc_2", "score": 0.6, "index": 2}]
...
def test_rrf_k_effect():
    ...
    assert spread_high < spread_low, (
        f"Expected high-k spread ({spread_high:.6f}) < low-k spread ({spread_low:.6f})"
    )
